...
export const CodeBlock = memo(function CodeBlock({ label, content, lang }: { label?: string; content: string; lang?: string }) {
  ...
  return (
    <div ref={blockRef} className="code-panel relative">
      ...
      {highlightedHtml ? (
        <div className={`code-panel-pre shiki-wrapper ${lang === 'json' ? 'text-[13px]' : 'text-sm'}`} dangerouslySetInnerHTML={{ __html: highlightedHtml }} />
      ) : (
        <pre className={`code-panel-pre ${lang === 'json' ? 'text-[13px]' : 'text-sm'}`}>
          <code>{content}</code>
        </pre>
      )}
    </div>
          ...
          <div className="flex-1 overflow-auto p-5">
            <div className="rounded-xl border border-border overflow-hidden h-full flex flex-col">
              ...
              <div className="flex-1 overflow-auto">
                {response ? (
                  <pre className="p-4 text-[15px] text-foreground leading-relaxed whitespace-pre-wrap" style={{ fontFamily: 'var(--font-mono)' }}>
                    <code>{response}</code>
                  </pre>
                ) : (
